...
import { AppEvent, ClickedLogInEvent } from "../../app.events";
import { browserLocalPersistence, getAuth, setPersistence, signInWithEmailAndPassword, signOut } from "firebase/auth";

export async function login(c: AppContext, ev: AppEvent): Promise<void> {
  const auth = getAuth();
  ...
  if (auth.currentUser) return; // User is already logged in

  if (!(ev instanceof ClickedLogInEvent)) throw new Error('Must be ClickedLogInEvent (Error can occur on initial load of app)');
